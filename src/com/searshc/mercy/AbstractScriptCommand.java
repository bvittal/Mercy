...
public abstract class AbstractScriptCommand
{
  ...
  */
  ...
  protected void toOrderResponse(MercyJsonObject obj)
  {
  ...
  protected void toOrder(MercyJsonObject obj, boolean useDefaults)
  {
    ...
    {
      MercyJsonObject mercyJsonObj = jasonObjItr.next();
     
    if(mercyJsonObj != null && mercyJsonObj.getName().equalsIgnoreCase(BASE_UNUSED_COUPON_CODE))
    {
    ...
    else if(mercyJsonObj != null && mercyJsonObj.getName().equalsIgnoreCase(BASE_INCOMPATIBILITIES))
    {
      ...
      {
        MercyJsonObject subMercyJsonObj = incompatibilityObjItr.next();
        incompatibilityList.add(this.getIncompatibilities(subMercyJsonObj));
      }
    }
  }
    ...
    if(map != null)
    {
      ...
      /** the parameter lists */
      List<MercyJsonArray> arrayList = obj.getMercyJsonArray();
      
      for(int i=0; i<arrayList.size(); i++)
      {
        String name = arrayList.get(i).getName();
        
        if(name.equals(MercyJsonArray.COUPON_KEY))
        {
        ...
        else if(name.equals(MercyJsonArray.OPTED_PROMO_KEY))
        {
    ...
    if (obj.getMercyJsonObject().size() > 0)
    {
        List<MercyJsonObject> itemList = obj.getMercyJsonObject();
        
        for(MercyJsonObject itemObj : itemList){
          List<MercyJsonObject> subItemList = itemObj.getMercyJsonObject();
          
          for(MercyJsonObject subItemObj : subItemList){
      ...
      while (itemIterator.hasNext())
      {
        ...
        Iterator<MercyJsonObject> itr = lineItems.getMercyJsonObject().iterator();

        while (itr.hasNext())
        {
          MercyJsonObject adjustmentObj = itr.next();
          if (adjustmentObj != null)
        {
          Iterator<MercyJsonObject> baseAdjustmentItr =  adjustmentObj.getMercyJsonObject().iterator();
          ...
          {
            if (adjustmentObj.getName().equalsIgnoreCase(MercyPojo.ADJUSTMENTS_KEY))
            {
              adjustments.add(this.getAdjustments(adjustmentMap));
              item.setAdjustments(adjustments);
            }
            
          else if (adjustmentObj.getName().equalsIgnoreCase(MercyPojo.DCADJUSTMENTS_KEY))
          {
  ...
  private Incompatibility getIncompatibilities(MercyJsonObject incompatibilityObj){
    ...
    if(incompatibilityObj != null && incompatibilityObj.getName().equalsIgnoreCase(SUB_BASE_INCOMPATIBILITY)){
      ...
      Iterator<MercyJsonObject> unappliedOffersItr = incompatibilityObj.getMercyJsonObject().iterator();
      while(unappliedOffersItr.hasNext()){
        MercyJsonObject unappliedOfferObj = unappliedOffersItr.next();
        Iterator<MercyJsonObject> unappliedOfferItr = unappliedOfferObj.getMercyJsonObject().iterator();

        while(unappliedOfferItr.hasNext()){
          MercyJsonObject unappliedOffersMercyJsonObj = unappliedOfferItr.next();
          if(unappliedOffersMercyJsonObj != null && 
              unappliedOffersMercyJsonObj.getName().equalsIgnoreCase(INCOMPATIBILITY_UNAPPLIED_OFFERS)){
              HashMap<String,String> unappliedOffersMap = unappliedOffersMercyJsonObj.getParameters();
        ...
        Iterator<MercyJsonObject> subUnappliedOfferItr = unappliedOffersMercyJsonObj.getMercyJsonObject().iterator();
        while(subUnappliedOfferItr.hasNext()){
          MercyJsonObject reasonsMercyJsonObj = subUnappliedOfferItr.next();
          Iterator<MercyJsonObject> reasonOfferItr = reasonsMercyJsonObj.getMercyJsonObject().iterator();
          
          while(reasonOfferItr.hasNext()){
            MercyJsonObject reasonMercyJsonObj = reasonOfferItr.next();
            AppMessage appMessage = new AppMessage();
            
            if(reasonMercyJsonObj != null && reasonMercyJsonObj.getName().equalsIgnoreCase(INCOMPATIBILITY_REASON)){
              HashMap<String,String> reasonsOffersMap = reasonMercyJsonObj.getParameters();
            ...
            Iterator<MercyJsonObject> fieldsItr = reasonMercyJsonObj.getMercyJsonObject().iterator();
            while(fieldsItr.hasNext()){
                MercyJsonObject fieldsMercyJsonObj = fieldsItr.next();
                List<String> fieldsList = new ArrayList<String>();
                
                if(fieldsMercyJsonObj != null && fieldsMercyJsonObj.getName().equalsIgnoreCase(INCOMPATIBILITY_FIELDS)){
                  HashMap<String,String> fieldsMap = fieldsMercyJsonObj.getParameters();
